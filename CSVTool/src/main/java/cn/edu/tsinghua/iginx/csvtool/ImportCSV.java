 ...
 * under the License.
 */
package cn.edu.tsinghua.iginx.csvtool;
...
public class ImportCSV extends AbstractCSVTool {
  ...
  private static void processSingleFile(String fileName) {
    ...
    try {
      ...
      if (!headerNames.get(0).equalsIgnoreCase("key")) {
        System.out.printf(
            "[ERROR] The first column must be named as [Key]! [%s] doesn't satisfy the requirement!%n",
            fileName);
        return;
      }
