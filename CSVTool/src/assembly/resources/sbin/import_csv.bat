...
pushd %~dp0..
if NOT DEFINED IGINX_CSV_IMPORTER_HOME set IGINX_CSV_IMPORTER_HOME=%CD%
popd

if NOT DEFINED MAIN_CLASS set MAIN_CLASS=cn.edu.tsinghua.iginx.csvtool.ImportCSV
if NOT DEFINED JAVA_HOME goto :err

@REM -----------------------------------------------------------------------------
@REM JVM Opts we'll use in legacy run or installation
set JAVA_OPTS=-ea^
 -DIGINX_CSV_IMPORTER_HOME="%IGINX_CSV_IMPORTER_HOME%"

REM For each jar in the IGINX_CSV_IMPORTER_HOME lib directory call append to build the CLASSPATH variable.
set CLASSPATH="%IGINX_CSV_IMPORTER_HOME%\lib\*"
