...
if [ -z "${IGINX_CSV_IMPORTER_HOME}" ]; then
  export IGINX_CSV_IMPORTER_HOME="$(
    cd "$(dirname "$0")"/..
    pwd
  )"
fi

MAIN_CLASS=cn.edu.tsinghua.iginx.csvtool.ImportCSV

CLASSPATH=""
for f in ${IGINX_CSV_IMPORTER_HOME}/lib/*.jar; do
