...
if [ -z "${IGINX_CSV_EXPORTER_HOME}" ]; then
  export IGINX_CSV_EXPORTER_HOME="$(
    cd "$(dirname "$0")"/..
    pwd
  )"
fi

MAIN_CLASS=cn.edu.tsinghua.iginx.csvtool.ExportCSV

CLASSPATH=""
for f in ${IGINX_CSV_EXPORTER_HOME}/lib/*.jar; do
...
case "${PARAMETERS[@]}" in
*"-tf "*) PARAMETERS=$PARAMETERS ;;
*) PARAMETERS=("-tf" "" "${PARAMETERS[@]}") ;;
esac
case "${PARAMETERS[@]}" in
*"-tp "*) PARAMETERS=$PARAMETERS ;;
*) PARAMETERS=("-tp" "ms" "${PARAMETERS[@]}") ;;
esac
