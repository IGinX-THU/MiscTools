...
class IGinXMetaServer(object):
    ...
    def get_fragments(self):
        ...
        for fragment in self.payload['fragments']:
            key_range = KeyRange(fragment['startKey'], fragment['endKey'])
            column_range = ColumnRange(fragment.get('startColumn', ColumnRange.UNBOUNDED_FROM),
                                       fragment.get('endColumn', ColumnRange.UNBOUNDED_TO))
            fragments.append(Fragment(key_range, column_range, fragment['storageUnitId']))
        return fragments
